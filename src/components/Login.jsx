...
const Login = () => {
    ...
    const Item = ({ ok, text }) => (
        <div className={`flex   gap-2 text-xs sm:text-sm ${ok ? "text-green-600" : " text-red-600"}`}>
    ...
    <div className='Netflix bg-[url(/assets/main_Bg.jpg)] bg-cover overflow-hidden '>
        <div className='  bg-black/50 flex justify-center w-full min-h-screen bg-cover '>
            <div className='w-full h-full flex flex-col  gap-10 items-center '>
                ...
                {/* Body */}
                <div className='w-70 sm:w-100 h-auto rounded-md bg-black/80 text-white  p-10 flex flex-col mt-30 sm:mt-40 gap-5 sm:gap-10'>
                    <h1  className='text-2xl sm:text-4xl font-extrabold'>{signIn? "Sign In": "Sign Up"}</h1>
                    <div >   
                        <form className='flex flex-col gap-5 w-full h-auto' action="">
                            {!signIn && <input ref={name} onClick={()=> {setShowsPasswordCheckList(false)}}  className='border border-gray-500 p-2 sm:p-4 outline-none rounded-md bg-[#242834]/50 ' type="text" placeholder='Full Name' name="" id="" />}
                            <input ref={email} onClick={()=> {setShowsPasswordCheckList(false)}} className='border border-gray-500 p-2 sm:p-4 outline-none rounded-md bg-[#242834]/50 '  type="email" placeholder='Email' name="" id="email" />
                            <input ref={password} onClick={showPasswordCheckList} onChange={(e) => setPasswordValue(e.target.value)} className='border border-gray-500 p-2 sm:p-4 outline-none rounded-md bg-[#242834]/50 ' type="password" placeholder='Password' name="" id="password" />
                    ...
                    <h1 className='text-gray-400 text-sm sm:text-lg '>{signIn? "New to Netflix? ":"Already registered? "  } <span onClick={toggleSigninForm} className='text-white cursor-pointer hover:underline'>{signIn? "Sign up now.":"Sign In now"}</span></h1>
                </div>
            </div>
        </div>
    </div>
